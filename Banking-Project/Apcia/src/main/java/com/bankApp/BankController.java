...
public class BankController {
    ...
    public void createUser() {
        ...
        if (socialId.equalsIgnoreCase("0")) {
            view.displayErrorMessage("You have already chose to exit from the creatUser menu!");
            return;
        }
        if ((Year.now().getValue() - Integer.parseInt(socialId.substring(0, 4))) < BankModel.VALIDAGE) {
    ...
    public void transfer() {
        ...
        User recipient = model.getUser(recipientSocialId);
