...
public class WithdrawMenu extends javax.swing.JDialog implements Serializable {
    ...
    private BankController bank;
         ...
         */
        public WithdrawMenu(java.awt.Frame parent, boolean modal, BankController bank, User customer) {
            super(parent, modal);
            initComponents();
        ...
        private void withdrawalButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_withdrawalButtonActionPerformed
            ...
            if (amountField.getText().isEmpty()) {
                ...
                double amount = 0;
                try {
                    amount = BankAccount.round(Double.parseDouble(amountField.getText()), 2);
                    int result = JOptionPane.showConfirmDialog(this, "Withdraw €" + String.format("%.2f", amount) + " from the account?\nTransaction Fee: $" + String.format("%.2f", bank.getTransaction(customer.getBankAccount().getAccountType())));
                    if (result == JOptionPane.OK_OPTION) {
                        try {
                            bank.withdraw(customer.getUsername().getAccountNumber(), amount);
                            this.dispose();
